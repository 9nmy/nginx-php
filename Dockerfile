...
RUN sed -i 's/dl-cdn.alpinelinux.org/mirrors.aliyun.com/g' /etc/apk/repositories \
        && apk add --no-cache --virtual .persistent-deps ca-certificates curl pcre zlib freetype libpng jpeg libcrypto1.1 libssl1.1 libressl libstdc++ gettext bison \
        && set -xe \
...
#开始安装php
        ...
        && apk add --no-cache --virtual .php-deps autoconf dpkg-dev dpkg file g++ gcc libc-dev make pkgconf re2c coreutils curl-dev freetype-dev libpng-dev jpeg-dev zlib-dev libedit-dev libressl-dev libsodium-dev libxml2-dev gettext-dev sqlite-dev  \
        \
